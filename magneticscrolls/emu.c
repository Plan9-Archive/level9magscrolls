...
type16 show_hints_text(struct ms_hint* hints, type16 index)
{
	...
	while (1)
	{
		...
		case 1: /* folders */
			output_text((unsigned char*)"Hint categories:\n");
			...
			output_text((unsigned char*)"Enter hint category number, ");
			if (hint->parent != 0xffff)
				output_text((unsigned char*)"P for the parent hint menu, ");
			output_text((unsigned char*)"or E to end hints.\n");
 ...
 */
	...
	if ((byte2 < 0xdd) || (version < 4 && byte2 < 0xe4) || (version < 2 && byte2 < 0xed))
	{
		...
		{
			if ((l1c = ms_undo()) != 0)
				output_text((unsigned char*)undo_ok);
			else
				output_text((unsigned char*)undo_fail);
			if (!l1c)
				write_reg(1, 2, '\n');
		}
	}
			...
			case 10: /* Open window commands */
				...
				case 4: /* Help/Hints */
					if (hints != 0)
					{
						...
						output_text((unsigned char*)no_hints);
					break;
				case 0: /* Carried items */
				case 1: /* Room items */
				case 2: /* Map */
				case 3: /* Compass */
					output_text((unsigned char*)not_supported);
					break;
				}
				break;
